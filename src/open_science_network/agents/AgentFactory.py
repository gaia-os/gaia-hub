...
class AgentFactory:
    ...
    def create(data, verbose=False):
        ...
        agents = AgentFactory.get_all_classes(agent_dir, "open_science_network.agents.impl.").values()
        ...
        interventions = {
            value for strategy in data.project.strategies for key, value in strategy.interventions.items()
        }
        if verbose:
        ...
        species = {value for strategy in data.project.strategies for value in strategy.species}
        if verbose:
