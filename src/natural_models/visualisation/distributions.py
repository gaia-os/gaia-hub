...
def compare_posteriors(
    is_observed, mcmc_samples=None, svi_samples=None, prediction_samples=None,
    labels=None, var_names=None, var_labels=None
):
    """
    Compare the variational posterior with posterior obtained from Monte Carlo Markov chain
    ...
    # Create the plot that will contain the plots displaying the posterior distributions
    fig, axes = plt.subplots(len(var_names), 1, figsize=(16, 10), sharex=True)
    ...
    for i, (var_name, var_label) in enumerate(zip(var_names, var_labels)):
        ...
        if f'obs_{var_name}' in prediction_samples.keys():
            draw_chart(axes[i], prediction_samples[f'obs_{var_name}'], labels[3], color, True, 3)
