...
import json
from natural_models.pydantic.Project import Project
...
class DataLoader:
    ...
    def __init__(self, project_file, reports_dir=None, verbose=False):
        ...
        self.policy = [strategies_by_name[lot.strategy].policy for lot in self.lots]
    ...
    def load_reports(self, report_dir):
        ...
        files = filter(lambda file: file is not None, sorted(files))
        ...
        for file in files:
            ...
            for lot in lots:
                ...
                entry = [report.id, report.datetime, location, report.reporter, t, lot] + \
                        [obs.value for obs in report.observations if obs.lot_name == self.lots[lot].name]

                new_report = gpd.GeoDataFrame(data=dict([(key, val) for key, val in zip(cols, entry)]), geometry='location')
        ...
        if self.verbose:
            print(f"Reports' columns: {', '.join(reports.columns.tolist())}")
            print("Reports' rows: \n")
