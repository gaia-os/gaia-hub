...
from natural_models.ontology.v1.management.base.agriculture.BioChar import UseBioChar as UseBioChar
from natural_models.ontology.v1.management.base.agriculture.Planting import PlantingSeeds as PlantingSeeds
from natural_models.ontology.v1.management.base.agriculture.Fertilizer import FertilizeSoil as FertilizeSoil
from natural_models.ontology.v1.management.base.agriculture.AlfalfaCompost import UseAlfalfaCompost as UseAlfalfaCompost
from natural_models.ontology.v1.management.base.agriculture.VermiCompost import UseVermiCompost as UseVermiCompost
from natural_models.ontology.v1.management.base.agriculture.CowManure import UseCowManure as UseCowManure
from natural_models.ontology.v1.management.base.agriculture.Mycorrhizae import UseMycorrhizae as UseMycorrhizae
from natural_models.ontology.v1.management.base.agriculture.Irrigation import IrrigateCrops as IrrigateCrops
from natural_models.ontology.v1.management.base.agriculture.Pesticide import SpreadPesticide as SpreadPesticide
from natural_models.ontology.v1.management.base.agriculture.Pruning import PruneCrops as PruneCrops
...
from natural_models.models.likelihoods import ElementSix as Likelihood
from abc import ABC
...
class AbstractRootsAndCultureAgent(AgentInterface, ABC):
    ...
    species = [
        ...
        PlantSpecies.__module__ + "." + PlantSpecies.__name__ + ".Alfalfa"
    ]

    # The model's actions
    actions = [
        HarvestCrops.__module__ + "." + HarvestCrops.__name__ + ".Hemp",
        HarvestCrops.__module__ + "." + HarvestCrops.__name__ + ".Alfalfa",
        UseBioChar.__module__ + "." + UseBioChar.__name__ + ".Yes",
        PlantingSeeds.__module__ + "." + PlantingSeeds.__name__ + ".HempSeeds",
        PlantingSeeds.__module__ + "." + PlantingSeeds.__name__ + ".AlfalfaSeeds",
        FertilizeSoil.__module__ + "." + FertilizeSoil.__name__ + ".No",
        UseAlfalfaCompost.__module__ + "." + UseAlfalfaCompost.__name__ + ".Yes",
        UseVermiCompost.__module__ + "." + UseVermiCompost.__name__ + ".Yes",
        UseCowManure.__module__ + "." + UseCowManure.__name__ + ".Yes",
        UseMycorrhizae.__module__ + "." + UseMycorrhizae.__name__ + ".Yes",
        IrrigateCrops.__module__ + "." + IrrigateCrops.__name__ + ".Yes",
        SpreadPesticide.__module__ + "." + SpreadPesticide.__name__ + ".Yes",
        PruneCrops.__module__ + "." + PruneCrops.__name__ + ".Yes"
    ]

    def __init__(self, agent_name, data):
        # Call parent constructor
        super().__init__(agent_name, data)
        ...
        self.obs_to_sample_site = {
            "v1.measurement.base.agriculture.Yield.HempYield.Continuous": "obs_yield_density",
            "v1.measurement.base.agriculture.PlantHeight.HempHeight.Continuous": "obs_plant_height",
            "v1.measurement.base.agriculture.PlantDensity.HempDensity.Continuous": "obs_plant_density"
        }
    ...
    def create_reports(self):
        ...
        for t in range(1, self.data.T + 1):
            for lot_index in range(len(self.data.project.lots)):
                ...
                observations = []
                for obs_name in self.obs_to_sample_site.values():
                    observation = {
                        "name": obs_name,
    ...
    def pre_process(self, data):
        ...
        data.policy = jnp.stack(policies, -2)
        return data
    ...
    def global_parametric_model(self, data_level=3, mask=None, forecast=False, *args, **kwargs):
        ...
        with plate('num_lots', self.n_lots):
            ...
            with plate('num_weeks', length):
                plant = jnp.expand_dims(jnp.any(self.data.policy == self.name_integer['planting-hemp'], -1), -1)
                ...
                irrigate = jnp.any(self.data.policy == self.name_integer['irrigation'], -1)
...
class DeterministicRootsAndCultureAgent(AbstractRootsAndCultureAgent):
    ...
    def deterministic_dynamics(self, effects, coefficients, mask, data_level):
        ...
        scan(step_fn, init, (self.data.policy, effects['water'], effects['temp'], effects['seeding'], mask))
